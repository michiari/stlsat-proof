...
| Corrected sets `K(u)`, `N(u)`, `O(u)`, `M(u)`, and `S(u)`, and the soundness/completeness limits, [`main.tex:615–731`](human/stlsat/main.tex#L615) | Window and limit definitions in [`Stlsat/Jump/Tableau.lean`](Stlsat/Jump/Tableau.lean#L148) |
...
shared occurrences, nodes, expansion, trees, acceptance, models
          ...
          │
          └── Jump.Rule:  ordinary expansion + guarded STEP/JUMP
```
...
- Atomic constraints are abstracted by `AtomicSemantics`. Consequently, the
  correctness results apply to any nonempty valuation type and atomic
  satisfaction relation, not only real-valued inequalities.
